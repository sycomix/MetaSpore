...
def append_source_table(feature_config, name, datasource, default_columns=[]):
    ...
    if datasource.collection:
        source_name = f"{datasource.serviceName}_{datasource.collection}"
    if not feature_config.find_source(source_name):
        raise ValueError(f"source: {source_name} must set in services!")
    if columns := setDefault(datasource, "columns", default_columns):
...
def columns_has_key(columns, key):
    return any(key in field for field in columns) if columns and key else False
...
class OnlineGenerator(object):
    ...
    def gen_docker_compose(self):
        ...
        no_mode_service = not any(str(name).startswith("model") for name in dockers)
        if no_mode_service:
            dockers["model"] = \
        ...
        for name, info in dockers.items():
            ...
            if "volumes" in info:
                volumes.extend(
                    f"{os.getcwd()}/volumes/{value}"
                    for key, value in info.volumes.items()
                )
            if not volumes and str(name).startswith("model"):
                volumes.append(f"{os.getcwd()}/volumes/output/model:/data/models")
            online_docker_compose.add_service(
                name,
                f"container_{name}_service",
                image=info.image,
                volumes=volumes,
                ports=info.ports,
            )
        ...
        if online_docker_compose.services:
            for name, service in online_docker_compose.services.items():
                ...
                if name == "recommend" or not service.ports:
                    continue
                online_recommend_service.add_env(f"{name.upper()}_HOST", name)
                online_recommend_service.add_env(f"{name.upper()}_PORT", service.ports[0])
                ...
                if "then" in experiment_data:
                    for model_name in experiment_data.then:
                        if model_name in self.service_dict:
                            ...
                            raise ValueError(f"no found service from model name: {model_name}")
            ...
            for data in self.configure.layers:
                ...
                for experiment_name, rato in layer_config.data.items():
                    if experiment_name not in recommend_experiments:
                        raise ValueError(f"no experiment_name:{experiment_name} found")
    ...
    def process_scenes(self, layer_set, feature_config, recommend_config):
        ...
        summary_select = [
            f"source_table_summary.{field}" for field in self.summary_fields
        ]
        for field_item in iteminfo_columns:
            summary_select.extend(
                [
                    f"{iteminfo_service_name}.{field}"
                    for field in field_item.keys()
                    if field not in [self.user_key, self.item_key]
                ]
            )
        feature_config.add_feature(
            name=feature_iteminfo_name,
            depend=[iteminfo_service_name, "source_table_summary"],
            select=summary_select,
            condition=[
                Condition(
                    left=f"{iteminfo_service_name}.{self.item_key}",
                    type="left",
                    right=f"source_table_summary.{self.item_key}",
                )
            ],
        )
            ...
            for data in self.configure.scenes:
                ...
                for layer_name in scene_data.layers:
                    if layer_name not in layer_set:
                        print(f"no layer_name:{layer_name} found")
                        continue
                ...
                if "additionalRecalls" in scene_data and scene_data.additionalRecalls:
                    ...
                    for model_name in scene_data.additionalRecalls:
                        if model_name not in random_recall_dict:
                            raise ValueError(f"random_recall_model: {model_name} not set in config!")
    ...
    def add_item_summary(self, feature_config):
        feature_config.add_feature(
            ...
            condition=[
                Condition(
                    left=f"source_table_request.{self.item_key}",
                    type="left",
                    right=f"source_table_summary.{self.item_key}",
                )
            ],
    ...
    def add_user_profile(self, feature_config):
        ...
        feature_config.add_feature(
            name="feature_user",
            depend=["source_table_request", "source_table_user"],
            select=[f"source_table_user.{field}" for field in self.user_fields],
            condition=[
                Condition(
                    left=f"source_table_request.{self.user_key}",
                    type="left",
                    right=f"source_table_user.{self.user_key}",
                )
            ],
        )
        user_key_action = FieldAction(
            names=[f"typeTransform.{self.user_key}"],
            types=["str"],
            fields=[self.user_key],
            func="typeTransform",
        )
        user_profile_actions = [
            ...
            FieldAction(
                ...
                func="splitRecentIds",
                fields=[self.items_key],
            ),
        ]
        user_profile_actions.append(
            FieldAction(
                names=[self.user_key, self.item_key, "item_score"],
                types=["str", "str", "double"],
                func="recentWeight",
                input=[f"typeTransform.{self.user_key}", "item_ids"],
            )
        )
    ...
    def add_one_random_recall(self, model_info, feature_config, recommend_config):
        ...
        if not columns_has_key(columns, key_name):
            raise ValueError(
                f"random model datasource column must has key_name: {key_name}!"
            )
        if not columns_has_key(columns, value_name):
            raise ValueError(
                f"random model datasource column must has value_name: {value_name}!"
            )
        append_source_table(feature_config, model_info.name, model_info.source, columns)
        random_bound = model_info.bound
        if random_bound <= 0:
            random_bound = 10
        random_task_name_1 = f"algotransform_random_{model_info.name}"
        ...
        random_task_name_2 = f"feature_random_{model_info.name}"
        feature_config.add_feature(
            name=random_task_name_2,
            depend=["source_table_request", random_task_name_1, model_info.name],
            select=[
                f"source_table_request.{self.user_key}",
                f"{random_task_name_1}.score",
                f"{model_info.name}.{value_name}",
            ],
            condition=[
                Condition(
                    left=f"{random_task_name_1}.hash_id",
                    right=f"{model_info.name}.{key_name}",
                )
            ],
        )
        field_actions = [
            FieldAction(
                names=[f"toItemScore.{self.user_key}", "item_score"],
                types=["str", "map_str_double"],
                func="toItemScore",
                fields=[self.user_key, value_name, "score"],
            )
        ]
        field_actions.append(
            FieldAction(
                names=[self.user_key, self.item_key, "score", "origin_scores"],
                types=["str", "str", "double", "map_str_double"],
                func="recallCollectItem",
                input=[f"toItemScore.{self.user_key}", "item_score"],
            )
        )
        random_task_name_3 = f"algotransform_recall_{model_info.name}"
    ...
    def add_one_cf_recall(self, model_info, feature_config, recommend_config):
        ...
        value_name = model_info.valueName
        if "columns" not in model_info.source:
            columns = [{key_name: "str"}, {value_name: {"list_struct": {"item_id": "str", "score": "double"}}}]
        else:
            columns = model_info.source.columns
        if not columns_has_key(columns, key_name):
            raise ValueError(f"cf model datasource column must has key_name: {key_name}!")
        for info in columns:
            if key_name in info:
                assert info[key_name] == "str"
        if not columns_has_key(columns, value_name):
            raise ValueError(
                f"cf model datasource column must has value_name: {value_name}!"
            )
        append_source_table(feature_config, model_info.name, model_info.source, columns)
        cf_task_name_1 = f"feature_cf_{model_info.name}"
        feature_config.add_feature(
            name=cf_task_name_1,
            depend=["algotransform_user", model_info.name],
            select=[
                f"algotransform_user.{self.user_key}",
                "algotransform_user.item_score",
                f"{model_info.name}.value",
            ],
            condition=[
                Condition(
                    left=f"algotransform_user.{self.item_key}",
                    type="left",
                    right=f"{model_info.name}.key",
                )
            ],
        )
        related_cf_task_name_1 = f"feature_related_cf_{model_info.name}"
        feature_config.add_feature(
            name=related_cf_task_name_1,
            depend=["source_table_request", model_info.name],
            select=[
                f"source_table_request.{self.user_key}",
                f"{model_info.name}.value",
            ],
        ...
        field_actions = [
            FieldAction(
                ...
                types=["str", "map_str_double"],
                func="toItemScore",
                fields=[self.user_key, "value", "item_score"],
            ),
            FieldAction(
                names=[self.user_key, self.item_key, "score", "origin_scores"],
                types=["str", "str", "double", "map_str_double"],
        ...
        cf_task_name_2 = f"algotransform_{model_info.name}"
        feature_config.add_algoTransform(name=cf_task_name_2,
                                         taskName="ItemMatcher", feature=[cf_task_name_1],
                                         options={"algo-name": model_info.name},
                                         fieldActions=field_actions,
                                         output=[self.user_key, self.item_key, "score", "origin_scores"])
        related_field_actions = [
            FieldAction(
                names=[f"toItemScore.{self.user_key}", "item_score"],
                types=["str", "map_str_double"],
                input=["setValue.item_score"],
                func="toItemScore",
                fields=[self.user_key, "value"],
            ),
            FieldAction(
                names=[self.user_key, self.item_key, "score", "origin_scores"],
                types=["str", "str", "double", "map_str_double"],
            ...
            FieldAction(
                names=["setValue.item_score"],
                types=["double"],
                func="setValue",
                options={"value": 1.0},
            ),
        ]
        related_cf_task_name_2 = f"algotransform_related_{model_info.name}"
        feature_config.add_algoTransform(name=related_cf_task_name_2,
                                         ...
                                         output=[self.user_key, self.item_key, "score", "origin_scores"])
        service_name = f"recall_{model_info.name}"
        if "recallService" in model_info and model_info.recallService:
            service_name = model_info.recallService
        recommend_config.add_service(name=service_name, tasks=[cf_task_name_2],
                                     options={"maxReservation": 200})
        self.service_dict[service_name] = service_name
        related_service_name = f"related_{model_info.name}"
