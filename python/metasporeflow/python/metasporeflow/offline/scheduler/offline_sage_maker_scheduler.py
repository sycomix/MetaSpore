...
class OfflineSageMakerScheduler(Scheduler):
    ...
    def _s3_work_dir(self):
        sage_maker_config = self._sage_maker_config
        return sage_maker_config.s3WorkDir

    @property
    def _crontab_expr(self):
        return self._scheduler_conf.data.cronExpr
    ...
    def _crontab_command(self):
        ...
        return f'{python} -m {module_name} --scene {scene_name} --redirect-stdio'

    @property
    def _crontab_entry(self):
        crontab_expr = self._crontab_expr
        crontab_command = self._crontab_command
        return f'{crontab_expr} {crontab_command}'
    ...
    def _s3_config_dir_path(self):
        ...
        return os.path.join(flow_dir, 'scene', scene_name, 'config')
    ...
    def _flow_config_path(self):
        ...
        return os.path.join(scene_dir, 'metaspore-flow.dat')
    ...
    def _generate_entrypoint(self, s3_config_dir_path):
        ...
        print(f'Generate SageMaker entrypoint to {s3_path} ...')
    ...
    def _upload_config(self):
        ...
        print(f'Upload algorithm config to {s3_path} ...')
    ...
    def _clear_config(self):
        s3_path = self._ensure_trailing_slash(self._s3_config_dir_path)
        print(f'Clear algorithm config {s3_path} ...')
    ...
    def _save_flow_config(self):
        ...
        print(f'Save MetaSpore flow config to {config_path} ...')
    ...
    def _clear_flow_config(self):
        ...
        config_path = self._flow_config_path
        config_dir = os.path.dirname(config_path)
        print(f'Clear MetaSpore flow config {config_path} ...')
    ...
    def _installed_in_crontab(self):
        ...
        lines = old_spec.splitlines()
        command = self._crontab_command
        return any(line.endswith(command) for line in lines)
    ...
    def _get_training_job_url(self, job_name):
        url = f'https://{self._aws_region}.console.amazonaws.cn'
        url += f'/sagemaker/home?region={self._aws_region}'
        url += f'#/jobs/{job_name}'
        return url
