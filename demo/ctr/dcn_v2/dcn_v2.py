...
sys.path.append('../../../')
from python.algos.dcn_v2_net import DCN
...
def init_spark():
    subprocess.run(['zip', '-r', 'demo/ctr/dcn_v2/python.zip', 'python'], cwd='../../../')
    spark_confs={
...
def train(spark, trian_dataset, **model_params):
    ...
    module = DCN(use_wide=use_wide, #True
                 ...
                 dnn_activations=dnn_activations, #'relu'
                 ...
                 model_structure=model_structure, #'stacked_parallel'
                 low_rank=low_rank, #32
                 num_experts=num_experts, #4
                 num_crossing_layers=num_crossing_layers, #3
                 use_low_rank_mixture=use_low_rank_mixture, #True
                 ftrl_l1=ftrl_l1,
                 ftrl_l2=ftrl_l2,
...
if __name__=="__main__":
    print('Debug -- CTR Demo Deep&Cross V2')
