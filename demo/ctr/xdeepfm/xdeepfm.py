...
sys.path.append('../../../') 
from python.algos.xdeepfm_net import XDeepFM
...
def init_spark():
    subprocess.run(['zip', '-r', 'demo/ctr/xdeepfm/python.zip', 'python'], cwd='../../../')
    spark_confs={
...
def train(spark, trian_dataset, **model_params):
    ...
    module = XDeepFM(use_wide=use_wide, #True
                    use_dnn=use_dnn, #True
                    use_cin=use_cin, #True
                    wide_embedding_dim=wide_embedding_dim, #10
                    deep_embedding_dim=deep_embedding_dim, #10
                    ...
                    dnn_hidden_units=dnn_hidden_units, #[1024, 1024, 1024]
                    dnn_hidden_activations=dnn_hidden_activations, #'relu'
                    use_bias=use_bias, #True
                    batch_norm=batch_norm, #True
                    net_dropout=net_dropout, #None
                    net_regularizer=net_regularizer, #None
                    cin_lay_unit=cin_lay_unit, #[16, 16, 16]
                    sparse_init_var=sparse_init_var, #1e-2 
                    embedding_regularizer=embedding_regularizer) #None
